...
None of these change the run's state machine or the STM command shapes themselves — they change what
value feeds `ROUND 2`, how `HOME` is tuned, and where arrow-read frames go. Apply each row's change
when implementing the task it names. Everything from this point to the end of the document is
shuenwei's original 2026-09-18 plan, unchanged.
