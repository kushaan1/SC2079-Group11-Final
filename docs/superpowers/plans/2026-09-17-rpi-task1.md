...
def test_defaults(monkeypatch):
    for name in ("BT_PORT", "STM_PORT", "PLANNER_URL", "STM_COMPLETION", "MOTOR_A_PCT"):
        monkeypatch.delenv("RPI_" + name, raising=False)
...
```bash
git add rpi/__init__.py rpi/config.py rpi/model.py rpi/requirements.txt rpi/requirements-dev.txt rpi/.gitignore rpi/tests/__init__.py rpi/tests/test_config.py rpi/tests/test_model.py
...
Run: `python -m pytest rpi/tests/test_protocol.py -q`
Expected: all passed (7 + 13 parametrised + 4 outbound = 34)
...
class BluetoothLink:
    ...
    def send(self, line: str) -> bool:
        """Write one line. Never raises; a failed write marks the link down."""
        payload = (line + "\n").encode("utf-8")
        with self._lock:
            fd = self._fd
            if fd is None:
...
- Produces:
  ...
  - `class StmDriver(ABC)`: `start()`, `manual(token)`, `manual_raw(line)`, `execute(instr)`, `stop()`, `close()`, property `available: bool`
...
class FakeStmDriver(StmDriver):
    ...
    def execute(self, instr: Instruction) -> None:
        ...
        if self._abort.is_set() or self._abort.wait(duration):
            self._abort.clear()
    ...
    def stop(self) -> None:
        self.sent.append("S")
        self._abort.set()
        # The abort window closes on its own, like the serial driver's resync does,
        # so a stop with nothing in flight cannot poison the next run.
        threading.Timer(0.5, self._abort.clear).start()
...
class SerialStmDriver(StmDriver):
    ...
    def _command(self, line: str, deadline_s: float, expect: Tuple[str, ...] = ("ACK,", "ERR,"), motion: bool = False) -> str:
        with self._lock:
            self._write(line)
            try:
                if motion and self._completion == "DONE":
                    first = self._await(("ACK,", "ERR,"), self._ack_deadline_s, line)
                    if first.startswith("ERR,"):
                        raise StmError(line, first)
                    final = self._await(("DONE,", "ERR,"), deadline_s, line)
                else:
    ...
    def manual_raw(self, line: str) -> None:
        self._command(line, self._ack_deadline_s)

    def execute(self, instr: Instruction) -> None:
        line = encode_instruction(instr)
        self._command(line, self._deadline(line), motion=True)
...
Timing notes: `test_start_without_pong_raises_unavailable` takes about 1.7 s (the grace period). If `test_serial_loss_marks_unavailable_and_reconnects` is ever flaky, raise its `wait_until` timeouts; it depends on the reader noticing the failed read (≤ 20 ms) and the reconnect loop's 50 ms tick.
...
Run: `python -m pytest rpi/tests -q`
Expected: all passed (10 + 18 + 34 + 7 + 40 + 4 + 7 + 1 = 121)
...
Pair the tablet once: `bluetoothctl` → `power on`, `discoverable on`, `pairable on`,
`agent on`, `default-agent`, then pair from the tablet and `trust <MAC>`.
...
- Produces:
  ...
  - `VisionWorker(client, send, frames_per_obstacle: int = 3)` with `start()`, `close()`, `submit(obstacle_id: int, frames: List[bytes], quiet: bool = False) -> None`, `wait(obstacle_id: int, timeout_s: float) -> Optional[Result]`, `wait_all(timeout_s: float) -> bool`, `results() -> List[Result]`, `pending -> int`
...
class VisionWorker:
    ...
    def submit(self, obstacle_id: int, frames: List[bytes], quiet: bool = False) -> None:
...
- Produces:
  - `_DrivingRun(BaseRun)` with `__init__(stm, camera, vision, send, state, radii, settle_s, frames)`, `_msg(text)`, `_report_pose(pose)`, `_drive_segment(segment, start, quiet=False) -> Tuple[str, Pose]` returning `("ok"|"stopped"|"failed", pose)`, `_capture(obstacle_id, quiet)`, and `submitted: List[int]`
...
def test_full_run_line_by_line():
    ...
    try:
        controller.start(run)
        assert wait_for(sent, "MSG,Capturing B2")
    ...
    assert sent == [
        ...
        "ROBOT,4.00,4.00,90",           # snap to the planner's capture pose
        "MSG,Capturing B1",
...
def test_stop_mid_segment():
    ...
    try:
        controller.start(run)
        assert wait_for(sent, "MSG,Visiting 2 of 2")
        time.sleep(0.05)
        controller.stop()
        controller.join(5.0)
    finally:
...
def test_done_reports_pending_verdicts_when_the_server_is_slow():
    vision = ScriptedVision({}, gate=threading.Event())    # never released
...
Extend the imports at the top of `rpi/run.py`:
...
class _DrivingRun(BaseRun):
    ...
    def _capture(self, obstacle_id: int, quiet: bool) -> None:
        self._msg("Capturing B%d" % obstacle_id)
        ...
        if frames:
            self._vision.submit(obstacle_id, frames, quiet=quiet)
...
Run: `python -m pytest rpi/tests -q`
Expected: all passed (121 + 19 + 10 + 12 + 10 + 6 + 9 + 3 = 190)
        ...
        try:
            ...
            while True:
                if segment is None:
                    ...
                    if outcome == "failed":
                        return
                    result = self._await_verdict(obstacle.obstacle_id)
                    if self.abort.is_set():
                        self._msg("Stopped")
                        return
    ...
    def _next_face(self, obstacle: Obstacle, pose: Pose, checked: set) -> Tuple[Optional[str], Optional[Segment]]:
        ...
        for face in FACES:
            ...
            if segment is None:
                checked.add(face)
                continue
            if best is None or segment.seconds < best[1].seconds:
                best = (face, segment)
...
Run: `python -m pytest rpi/tests -q`
Expected: all passed (190 + 6 + 1 = 197)
...
In §1.4 "Task control", change the start-image-recognition JSON example to include the robot object, and add the face-search message. Insert after the existing `#### Start image recognition` block's field table this row and note:
