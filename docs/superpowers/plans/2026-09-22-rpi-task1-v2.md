# RPi Program (Task 1 + A.5) Implementation Plan (v2, rules-checked)

**This is a 2026-09-22 revision of shuenwei's 2026-09-17 original**, copied unchanged below the line
...
`docs/superpowers/specs/2026-09-22-rpi-task1-design-v2.md`, for the full reasoning — this section only
maps the deltas onto where they land in *this* plan's tasks.

## Rule deltas vs. the 2026-09-17 original (added 2026-09-22; extended 2026-09-25)
...
`kejun-experimental-algo`, also checked 2026-09-25.

| # | Where in this plan | Original plan | What the rules require | Why |
|---|---|---|---|---|
| 1 | The vision worker task (`vision_worker.py`, `decide()`) — search this file for `Task` steps that build `rpi/vision_worker.py` | Reports the highest-confidence `target` verdict seen, with no minimum confidence | A correct image ID scores **+10**; a wrong one scores **−10** (`docs/rules/rules.md` FAQ 3–4). Below a confidence floor (`config.TARGET_MIN_CONFIDENCE`), `decide()` must fall through to `bullseye`/`no_detection` instead of reporting a low-confidence guess as `target` | Guessing risks turning a 0-point miss into a −10 penalty |
| 2 | The `Task1Run` task (`run.py`) — search for the steps that build `Task1Run.run()` | Drives the whole plan, then waits up to `VISION_DRAIN_TIMEOUT_S` for outstanding verdicts, with no ceiling tied to the competition clock | The robot must **stop itself, automatically, within 6 minutes** of the run starting (`docs/rules/rules.md` rule 10). `Task1Run` needs a `config.TASK1_TIME_LIMIT_S` (360s) deadline from `start_time = monotonic()`, checked before each segment and each verdict wait, that force-stops the robot and reports `Done` with whatever's in by then | A run that only stops because a human intervenes is marked "incomplete" and ranked below every team that stopped itself, regardless of what it recognised (`docs/rules/rules.md` FAQ 15) |
| 3 | **[found 2026-09-25]** No code change — a documentation-only gap | Not mentioned | Task 1 shares Task 2's "stay inside the yellow-box CARPARK zone during the 2-minute prep" rule (official PDF rule 3, FAQ 9: *"Yellow box for task 1"*) | Nothing in this plan drives the wheels before the tablet's `imageRec` start, so it's already satisfied in code — it's a caution for whoever tests manual-drive buttons during setup, not a build task |
| 4 | **[found 2026-09-25]** The `stm_driver.py` task's assumptions about `BW`, the `FW`/`BW` range, and ACK/DONE timing (search for "Open with the STM team" or the `S1`–`S4` table this plan inherited) | Presented as open questions the STM team hasn't answered | **All three are already confirmed** by reading `carl-stm`'s current `main.c` directly: `BW` exists and is calibrated (`BW_COUNTS_PER_100CM=7418`, dated 2026-09-20); the range is exactly 5–200; ACK arrives on receipt, `DONE` on completion. A fourth, `S4` (does an `S`-interrupted move still reply), is also now confirmed: no, it doesn't | Implement `stm_driver.py` against these as settled facts, not open assumptions — no behavior in this plan needs to change, only the confidence with which it's built |
| 5 | **[found 2026-09-25]** The `rpi/config.py` task's `TURN_RADIUS_CM` defaults (search for `_radii` or `TURN_RADIUS_CM` in the `config.py` task) | `{FL: 39, FR: 40, BL: 37, BR: 39}` — the prior-year reference team's numbers | `kejun-experimental-algo/algorithm/config.py` was updated 2026-09-18 with real measurements on this chassis: `{FL: 42, FR: 56, BL: 41, BR: 55}`. **Already applied directly to the committed `rpi/config.py` and its test on 2026-09-25** — this row is for anyone re-implementing from this plan document from scratch rather than from the current `rpi/` tree | Keeps the tablet's dead-reckoned marker (used only between segments, since each segment snaps to the planner's real pose) consistent with what the planner actually assumes |

Rows 1 and 2 are additions to code this plan already describes building — `vision_worker.decide()` and
`Task1Run.run()` — not new tasks or new files. Row 3 needs no code at all. Rows 4 and 5 don't change
what to build, only correct what this plan assumed was still open or still stale when it was written.
Everything from this point to the end of the document is shuenwei's original 2026-09-17 plan,
unchanged.
